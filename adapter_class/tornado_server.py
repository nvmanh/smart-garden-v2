...
def trigger_action(actor_type, actor_id, action_type, parameter):
    try:
        logging.info(
            "trigger_action::" + str(actor_type) + ' ' + str(actor_id) + ' ' + str(action_type) + ' ' + str(parameter))
        ...
        trigger = Action_Trigger(actor_type, actor_id)
        trigger.writeAction(actor_type, parameter)

        logging.info(
            "trigger_action written::" + str(actor_type) + ' ' + str(actor_id) + ' ' + str(action_type) + ' ' + str(
                parameter))
    except:
        exc_type, exc_value, exc_traceback = sys.exc_info()
...
def send_initial_data(client):
    try:
        ...
        feed = yield r.table("observations").order_by(r.desc('timestamp')).limit(200).order_by(r.asc('timestamp')).run(
            tconn)
